    ...
    if cond =="existe":
        ...
        if linha>=0 and linha<=(tamanho_dict-1):
            while True:
                ...
                if coluna in cabecalho and  coluna !="DATA DE VENCIMENTO":
                    while True:
                        novo_dado=input("Digite o novo dado:\n ('Use todas as letras em minúsculo.') ")
                        novo_dado = novo_dado.strip()
                        novo_dado2=novo_dado.islower()
                        if novo_dado2==True:
                            modificado = lupy.loc[linha,coluna]=novo_dado
                            print("\nDado modificado com sucesso!\n")
                            break
                        else:
                            print("\nATENÇÃO!!! Digite todas as letras minúsculas.\n")
                    break
                elif coluna in cabecalho and coluna=="DATA DE VENCIMENTO":
                    novo_dado=input("Digite o novo dado: ")
                    novo_dado = novo_dado.strip()
                    modificado = lupy.loc[linha,coluna]=novo_dado          
                    print("\nDado modificado com sucesso!\n")
...
def principal(cond):
    ...
    - Argumento de entrada:
        cond: variável atualizada na função de consulta.
    """
    lupy = pd.read_csv(r'C:\Users\erakt\Desktop/proj.csv',encoding = 'ISO-8859-1')
    lupy = lupy.rename(columns = {'SITUAÃÃO': 'SITUAÇÃO'})
    
    while True:
        opcao = interface()    
        if opcao == "4":
            print("\nPrograma encerrado.")
            export_csv = lupy.to_csv(r'C:\Users\erakt\Desktop/proj.csv',index=False, header = True)
            break
        else:
            if opcao == "1":
                print()
...
# em equipe.)
# Ao gif do panda que muito nos ajudou.
#--------------ATENÇÃO-------------#
# É preciso ter a biblioteca pandas instalado no computador
